...
Stable releases are read-only. Testing `0.0.59` can modify only the opted-in character's AutoRetainer embedded Venture Plan through AutoRetainer's public IPC after an authenticated Gillions request. The original plan and completion behavior are retained locally and on Gillions for verified restoration. A deliberate conflict retry is accepted only when current AutoRetainer state exactly matches the hash previously acknowledged to Gillions; another local change blocks the write. No AutoRetainer configuration file is read or written directly, and unrelated AutoRetainer settings are excluded from the backup and mutation.
