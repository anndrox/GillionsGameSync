...
## Pairing, storage and review for 1.0.30

The release includes the adopted one-time legacy re-pairing requirement, immutable issuing-origin binding, new generation/character partitions, exact-version acknowledgements and the shared 10,000-record/8 MiB pending-data budget. The user-facing notes must explain inactive old history, visible coverage gaps, prior generations still counting toward storage, the global Automatic sync control and the separate default-on item-link control. Do not reuse a retirement-only package or release body that says no re-pairing is needed.

Prepare one exact source/package candidate for consolidated UX, Security and Compatibility review. Supply the actual stable/testing serializer evidence, deterministic UI view-state fixtures and explicit live-game/visual limits. Required review acceptance precedes source integration and public publication. The normal server receipt remains unchanged; client Gil payloads omit absent numeric properties to avoid the existing server's null-to-zero normalization. No server migration or runtime operation is part of plugin packaging.

An older binary may ignore the new owned-state fields when saving configuration and can retain its former credential/planner behavior. A downgrade is therefore not a supported automatic recovery path: obtain a specific compatibility/rollback review before using one. Existing older installed clients remain a separate state; publication cannot prove their cancellation or upgrade. Preserve prepared, superseded and final candidate identities, and never replace an already published version's immutable artifact.
